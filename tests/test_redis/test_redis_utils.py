...
class TestRedisUtils:
    ...
    async def test_redis_set(self, mock_get_client, mock_redis):
        ...
        mock_redis.set.assert_called_with(
            "test_key", "test_value", ex=300, px=None, nx=False, xx=False
        )
    ...
    async def test_redis_expire(self, mock_get_client, mock_redis):
        ...
        mock_redis.expire.assert_called_with(
            "test_key", 300, nx=True, xx=False, gt=False, lt=False
        )
    ...
    async def test_redis_json_set(self, mock_get_client, mock_redis):
        ...
        assert result is True
        mock_redis.json_set.assert_called_with(
            "user:123", ".", test_data, nx=True, xx=False
        )
