...
class TestRedisIntegration:
    ...
    def test_get_redis_not_initialized(self):
        ...
        try:
            with pytest.raises(
                RedisConnectionError, match="Redis client not initialized"
            ):
                get_redis()
        finally:
    ...
    def test_redis_list_operations_route(self, test_client_with_redis, mock_redis):
        ...
        async def get_message_count(
            request: Request, response, redis=Depend(get_redis)
        ):
        ...
        response = test_client_with_redis.post(
            "/messages", json={"message": "Hello World"}
        )
    ...
    def test_redis_hash_operations_route(self, test_client_with_redis, mock_redis):
        ...
        async def update_profile(
            request: Request, response, user_id, redis=Depend(get_redis)
        ):
            data = await request.json
        ...
        # Test update profile
        response = test_client_with_redis.post(
            "/user/123/profile", json={"name": "John Doe", "email": "john@example.com"}
        )
        assert response.status_code == 200
