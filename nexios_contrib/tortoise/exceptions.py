...
def handle_tortoise_exceptions(app: "NexiosApp") -> None:
    ...
    async def handle_integrity(
        request: Request, response: Response, exc: IntegrityError
    ):
    ...
    async def handle_not_found(request: Request, response: Response, exc: DoesNotExist):
        ...
        return response.json(
            {"error": "Record not found", "detail": str(exc), "type": "not_found_error"}
        ).status(404)
    ...
    async def handle_operational(
        request: Request, response: Response, exc: OperationalError
    ):
