...
class Task:
    ...
    async def wait(self, timeout: Optional[float] = None) -> Any:
        ...
        try:
            ...
            if (
                self._result
                and self._status == TaskStatus.FAILED
                and self._result.error
            ):
                raise self._result.error
            return self._result.result if self._result else None
        except asyncio.TimeoutError:
            raise asyncio.TimeoutError(
                f"Task {self.id} timed out after {timeout} seconds"
            )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "duration": (
                (self._completed_at or time.time())
                - (self._started_at or self._created_at)
                if self._started_at
                else None
            ),
