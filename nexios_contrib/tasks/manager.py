...
class TaskManager:
    ...
    def __init__(
        self, app: Optional[NexiosApp] = None, config: Optional[TaskConfig] = None
    ) -> None:
    ...
    async def shutdown(self) -> None:
        ...
        tasks_to_cancel = [
            task for task in self.tasks.values() if task._task and not task.is_done
        ]
    ...
    async def create_task(
        ...
        **kwargs: Any,
        ...
        # Create the task
        task = Task(func, *args, name=name or func.__name__, **kwargs)  # ty:ignore[unresolved-attribute]
    ...
    async def _run_task(self, task: Task, timeout: Optional[float] = None) -> None:
        ...
        try:
            ...
            task._result = TaskResult(
                ...
                error=asyncio.CancelledError("Task was cancelled"),  # ty:ignore[invalid-argument-type]
    ...
    async def _execute_callbacks(self, task: Task) -> None:
        ...
        for callback in self._task_callbacks[task.id]:
            try:
                await callback(
                    task.id, task._result, task._result.error if task._result else None
                )
            except Exception as e:
                self.logger.exception(
                    "Error in task callback for %s: %s", task.id, str(e)
                )
    ...
    def add_callback(
        self, task: Union[str, Task], callback: TaskResultCallback
    ) -> None:
    ...
    def remove_callback(self, task_id: str, callback: TaskResultCallback) -> bool:
