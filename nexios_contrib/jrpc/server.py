...
class JsonRpcPluginConfig(TypedDict):
    path_prefix: str


class JsonRpcPlugin:
    """JSON-RPC server built on Starlette."""

    app: NexiosApp
    config: JsonRpcPluginConfig
    registry: JsonRpcRegistry

    def __init__(
        self,
        app: NexiosApp,
        config: JsonRpcPluginConfig = {"path_prefix": "/rpc"},
    ):
        self.app = app
        self.config = config
    ...
    async def handle_jsonrpc(self, req: Request, res: Response):
        ...
        try:
            ...
            if not method_name or not isinstance(method_name, str):
                raise JsonRpcInvalidRequest("Method name must be a string")

            params = data.get("params", {})
            if not isinstance(params, (dict, list)):
                raise JsonRpcInvalidRequest("Params must be an object or array")

            request_id = data.get("id")
            if request_id is not None and not isinstance(request_id, (str, int)):
                raise JsonRpcInvalidRequest("Id must be a string or number")
